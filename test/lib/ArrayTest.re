  ...
  describe("Getter and setter", ({describe, test}) => {
    describe("getExn", ({test}) => {
      test("doesn't throw", ({expect}) => {
        let arr = [|1, 2, 3|];
        expect.fn(() => getExn(arr, 1)).not.toThrow();
        expect.int(getExn(arr, 1)).toBe(2);
      });

      test("throws", ({expect}) => {
        let arr = [|1, 2, 3|];
        expect.fn(() => getExn(arr, 5)).toThrow();
      });
    });

    describe("setExn", ({test}) => {
      test("doesn't throw", ({expect}) => {
        let arr = [|1, 2, 3|];
        expect.fn(() => setExn(arr, 1, 1)).not.toThrow();
        expect.array(arr).toEqual([|1, 1, 3|]);
      });

      test("throws", ({expect}) => {
        let arr = [|1, 2, 3|];
        expect.fn(() => setExn(arr, 5, 1)).toThrow();
      });
    });
    ...
    test("getByi", ({expect}) => {
      let arr = [|1, 2, 3|];
      expect.fn(() => getByi(x => x == 1, arr)).not.toThrow();
      expect.option(getByi(x => x == 1, arr)).toBe(Some(0));
      expect.option(getByi(x => x == 5, arr)).toBeNone();
    });
  });

  describe("Manipulation", ({describe, test}) => {
    ...
    test("fillInPlace", ({expect}) => {
      let arr = [|1, 2, 3|];
      expect.fn(() => fillInPlace(1, 2, 1, arr)).not.toThrow();
    ...
    test("fill", ({expect}) => {
      let arr = [|1, 2, 3|];
      let newArr = ref([||]);
      expect.fn(() => newArr := fill(1, 2, 1, arr)).not.toThrow();
      expect.array(arr).toEqual([|1, 2, 3|]);
      expect.array(newArr^).toEqual([|1, 1, 1|]);
    });
